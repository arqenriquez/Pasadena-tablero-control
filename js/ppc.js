/* ================================================================
   PASADENA · TABLERO · Módulo 04 · PPC (Last Planner System)
...
function renderTabla() {
  ...
  const rows = S.metas.map((m, i) => {
    ...
    <tr class="${trClass}" data-idx="${i}">
      ...
      <td class="col-lote" data-label="Partida">${m.lote ? `<span class="ppc-lote-tag">${escapeHtml(m.lote)}</span>` : '—'}</td>
  ...
  host.innerHTML = `
    <div class="ppc-table-wrap">
      <table class="ppc-table">
        <thead><tr>
          <th>Actividad / Meta</th><th>Responsable</th><th class="col-lote">Partida</th><th class="col-check">Cumplida</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
...
function construirCierre() {
  ...
  return {
    ...
    fecha_cierre: hoyISO(),
    responsable: 'Arq. Jorge E. Enríquez',
    total: r.total,
...
function renderActa(cierre) {
  const nombreProy = S.proyecto?.proyecto?.nombre || 'Pasadena';
  ...
  $('#ppc-print').innerHTML = `
    <div class="acta-head">
      <div class="acta-head-left">
        ...
        <div>
          ...
          <div class="sub">${escapeHtml(nombreProy)} · Last Planner System</div>
    ...
    <table class="acta-table">
      <thead><tr><th>Actividad / Meta</th><th>Responsable</th><th>Partida</th><th>Cumplida</th><th>Motivo de incumplimiento</th></tr></thead>
      <tbody>${filas}</tbody>
    </table>
...
function pintarChart(puntos, modo) {
  ...
  S.chart = new Chart($('#ppcChart').getContext('2d'), {
    ...
    data: {
      labels,
      datasets: [
        { label: 'PPC semanal', data: semanal, borderColor: '#5a9bd4', backgroundColor: 'rgba(90,155,212,0.08)', borderWidth: 2.5, tension: 0.3, fill: true, spanGaps: false, pointRadius: muchas ? 3 : 4, pointBackgroundColor: '#5a9bd4' },
        { label: 'PPC acumulado', data: acum, borderColor: '#d4b876', borderDash: [6, 4], backgroundColor: 'transparent', borderWidth: 2.5, tension: 0.3, fill: false, spanGaps: false, pointRadius: muchas ? 2 : 3, pointBackgroundColor: '#d4b876' },
