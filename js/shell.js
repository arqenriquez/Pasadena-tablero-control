/* ============================================================
   PLANTILLA · TABLERO · Lógica de la página principal (index.html)
...
function renderProyecto(proy) {
  ...
  $('#hero-nombre').textContent = p.nombre || 'Cliente';
  if (p.descripcion) $('#hero-descripcion').textContent = p.descripcion;
  $('#meta-ubicacion').textContent = p.ubicacion || '—';
  if (p.semana_actual != null && p.total_semanas != null) {
    const sa = String(p.semana_actual).padStart(2, '0');
    const st = String(p.total_semanas).padStart(2, '0');
    $('#meta-semanas').textContent = `Semana ${sa} / ${st}`;
  } else {
    $('#meta-semanas').textContent = '—';
  }
  $('#meta-inicio').textContent = fechaLarga(p.fecha_inicio);
  $('#meta-fin').textContent = fechaLarga(p.fecha_fin);
  document.title = `${p.nombre || 'Cliente'} · Tablero de Control | Metta`;
